...
const colorLab = String.raw`(?:ok)?lab\(\s*${colorPercentage}\s+${colorNumber}\s+${colorNumber}(?:\s*\/\s*${colorAlpha})?\s*\)`;
...
const colorLCH = String.raw`(?:ok)?lch\(\s*${colorPercentage}\s+${colorNumber}\s+${colorHue}(?:\s*\/\s*${colorAlpha})?\s*\)`;
