...
test('Accepts valid defaultLanguage options', t => {
	t.is(parser({defaultLanguage: 'foo'}).defaultLanguage, 'foo');
	t.is(parser({defaultLanguage: 'inform7'}).defaultLanguage, 'inform7');
	t.is(parser({defaultLanguage: 'nand2tetris-hdl'}).defaultLanguage, 'nand2tetris-hdl');
});


test('Converts defaultLanguage option to lower case', t => {
	t.is(parser({defaultLanguage: 'HTML'}).defaultLanguage, 'html');
	t.is(parser({defaultLanguage: 'CSS'}).defaultLanguage, 'css');
	t.is(parser({defaultLanguage: 'javaScript'}).defaultLanguage, 'javascript');
});
